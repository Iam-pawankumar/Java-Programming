...
public class Aided extends Student {
    ...
    public String result(String allMarks) {
        ...
        nccMarks = marksSet[1].split(",");
        sportsMarks = marksSet[2].split(",");
        ...
        if(Integer.parseInt(nccMarks[0]) == 1){
            gradePoint +=  Double.parseDouble(grade(Double.parseDouble(nccMarks[1]))) * Double.parseDouble(nccMarks[2]);
            totalCredits += 5;
        }
